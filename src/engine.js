...
//			audio_el.playbackRate = 0.5;
		...
		app.listenFor('RequestBookmarkRegion', function () { // Toggle the button and set the boolean 
			if (!q.is_ready) return;

			if (wavesurfer.regions.list[0]) {
				const reg = wavesurfer.regions.list[0];
				// console.log(reg.start);
				// console.log(reg.end);
				
				const regName = `R ${app.engine.roundFloat2(reg.start)}-${app.engine.roundFloat2(reg.end)}`;
				...
				new PKSimpleModal({
					...
					buttons: [{
						...
						callback: function (q) {
							...
							const reg = app.engine.wavesurfer.regions.list[0];
							const id = app.getId()
							app.regionMate.regions[id] = {
								id,
								name: input.value.trim(), 
								startTime: reg.start,
								endTime: reg.end,
								startProg: reg.start / wavesurfer.getDuration(),
								endProg: reg.end / wavesurfer.getDuration(),
							};
							app.regionMate.refreshUI();
							q.Destroy();
						}
					}],
					body: `
					<div class="pk_row"><label for="k0">Region Name</label> 
						<input style="min-width:250px" placeholder="Region Name" value="${regName}" class="region_name" type="text" id="k0" /></div>
					`,
					setup: function (q) {
